...
{                          // OJO no tiene cierre esta llave
...
QMenu *corpusMenu = newManagedMenu(menu, "corpus", tr("Build Corpus"));
newManagedAction(corpusMenu, "buildandviewcorpus", tr("Build && View MD..."), SLOT(buildAndViewCorpus()), QKeySequence(), "buildcorpus");
newManagedAction(corpusMenu, "buildcorpus", tr("Build Only..."), SLOT(buildCorpus()), QKeySequence(), "buildcorpus");
menu->addSeparator();
...
} // <--- AQUÍ SE CIERRA CORRECTAMENTE setupMenus()
...
void Iguana::setupToolBars()
{
    ...
    for (int i = 0; i < configManager.managedToolBars.size(); i++) {
        ...
        foreach (const QString &actionName, mtb.actualActions) {
            if (actionName == "separator") {
                mtb.toolbar->addSeparator(); //Case 1: Separator
    ...
    addBuildCorpusToolbarButton();
} // ✅ Cierre correcto y balanceado de setupToolBars()
...
static QIcon tintIcon(const QIcon &base, const QColor &color, const QSize &size)
{
	if (base.isNull()) return base;
	const qreal dpr = qGuiApp ? qGuiApp->devicePixelRatio() : 1.0;
	const QPixmap pm = base.pixmap(size * dpr);
...
{
	if (findChild<QToolButton*>("corpusComboButton")) return;
	QAction *viewAct  = getManagedAction("main/tools/corpus/buildandviewcorpus");
	QAction *buildAct = getManagedAction("main/tools/corpus/buildcorpus");
	QAction *quickAct = getManagedAction("main/tools/quickbuild");
	if (!viewAct || !buildAct) { qWarning("corpus combo: acciones no encontradas"); return; }
	const QColor jacaranda(0x8A, 0x63, 0xD2);
	const QSize sz = iconSize();
	if (quickAct) viewAct->setIcon(tintIcon(quickAct->icon(), jacaranda, sz));
	if (QAction *cp = getManagedAction("main/tools/compile"))
		buildAct->setIcon(tintIcon(cp->icon(), jacaranda, sz));
	QToolBar *bar = nullptr;
	QAction *greenAction = nullptr;
	for (QToolBar *tb : findChildren<QToolBar*>()) {
		for (QAction *a : tb->actions()) {
			QToolButton *btn = qobject_cast<QToolButton*>(tb->widgetForAction(a));
			if (btn && btn->menu() && quickAct) {
				for (QAction *ma : btn->menu()->actions()) {
					if (ma->text() == quickAct->text()) { bar = tb; greenAction = a; break; }
				}
			}
			if (bar) break;
		}
	...
	QToolButton *combo = new QToolButton(bar);
	combo->setObjectName("corpusComboButton");
	combo->setDefaultAction(viewAct);
	combo->setPopupMode(QToolButton::MenuButtonPopup);
	QMenu *m = new QMenu(combo);
	m->addAction(buildAct);
	combo->setMenu(m);
	const QList<QAction*> acts = bar->actions();
	const int i = acts.indexOf(greenAction);
	QAction *after = (i >= 0 && i + 1 < acts.size()) ? acts.at(i + 1) : nullptr;
	if (after) bar->insertWidget(after, combo);
	else bar->addWidget(combo);
	qWarning("corpus combo: montado en %s", qPrintable(bar->objectName()));
}
